...
class iarduino_nLED{																//
	...
	/**	функции доступные пользователю **/											//
		...
		void			print ( uint32_t    num, uint8_t param=255)					{	char i[11]; ultoa(num,i,param==BIN?BIN:(param==OCT?OCT:(param==HEX?HEX:DEC))); prnSTR(i,1,len_DAT,pos_ALG,pos_SYM); } // 0...+4294967295
		void			print ( int32_t     num, uint8_t param=255)					{	char i[12]; ltoa (num,i,param==BIN?BIN:(param==OCT?OCT:(param==HEX?HEX:DEC))); prnSTR(i,1,len_DAT,pos_ALG,pos_SYM); } // 0...±2147483648
		void			print ( double      num, uint8_t param=255)					{	char i[21]; if(param==255){param=2;} if(param>9){param=9;} int32_t j=num; uint32_t k=(fabs(num)-fabs(j))*1000000000; ltoa(j,i,10); if(param){uint8_t f=strlen(i); i[f]='.'; ltoa(k,&i[f+1],10); i[f+param+1]=0;} prnSTR(i,1,len_DAT,pos_ALG,pos_SYM);}
		void			print ( char*       str, uint8_t param=255)					{	prnSTR(str,1,len_DAT,pos_ALG,pos_SYM);}
		void			print ( const char* str, uint8_t param=255)					{	prnSTR((char*)str,1,len_DAT,pos_ALG,pos_SYM);}
		void			print ( String      str, uint8_t param=255)					{	uint8_t j=str.length()+1; char i[j]; str.toCharArray(i, j); prnSTR(i,1,len_DAT,pos_ALG,pos_SYM); }
																					//
